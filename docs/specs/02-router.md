...
**モジュール使用時の設定:**
- Horizonモジュールを依存関係として使用する場合、PCRE2のリンク設定は自動的に適用されます
- `build.zig`のhorizonモジュール定義で`link_libc = true`と`linkSystemLibrary("pcre2-8")`が設定されています
- 利用側プロジェクトで明示的にPCRE2をリンクする必要はありません
- ただし、システムにPCRE2ライブラリ（libpcre2-8）がインストールされている必要があります
